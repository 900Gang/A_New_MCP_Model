...
| A8 | 2026-09-16 | §4.2, §11.6 | Lint and type gates extended to cover the repository-root `scripts/` directory. | Ruff and mypy were scoped to `backend/` only, so `scripts/check_codeowners.py` and any future root script sat outside every gate. Running ruff against it manually found two real defects in a file that had already been committed. A Python file outside the gates is a gate gap, not an exception. |
